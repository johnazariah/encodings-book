...
(H\textsubscript{2}) in the STO-3G basis, we construct the qubit Hamiltonian explicitly
...
encodings, reduce qubit count via diagonal and Clifford Z\textsubscript{2} symmetry
...
Fock-space operator algebra. Two running examples --- H\textsubscript{2} (4 qubits) and
H\textsubscript{2}O (12--14 qubits) --- are developed from molecular geometry to quantum
circuit, including computing the H\textsubscript{2} dissociation curve and the H-O-H
...
\bottomrule\noalign{}
\endlastfoot
\texttt{diagonalZ2SymmetryQubits} & Find diagonal Z\textsubscript{2} qubits \\
\texttt{taperDiagonalZ2} & Apply diagonal tapering \\
\texttt{findCommutingGenerators} & Find all Z\textsubscript{2} symmetries \\
...
\tightlist
\item
  \textbf{Unary:} \(d\) qubits, weight $\leq$ 2
\item
