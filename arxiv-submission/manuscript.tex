...
\title{From Molecules to Quantum Circuits}
\usepackage{etoolbox}
\makeatletter
...
Every chemistry textbook tells you that water's bond angle is 104.5\textdegree{} and
...
(H\textsubscript{2}), because it is the simplest system that exhibits all the essential
structure, and the water molecule (H\textsubscript{2}O), because it is the most
...
boils at 100\textdegree{}C rather than -50\textdegree{}C. It is arguably the single most
...
We will do this for the hydrogen molecule, H\textsubscript{2}. Not because H\textsubscript{2} is
interesting in itself (it isn't --- any laptop can solve H\textsubscript{2} exactly in
milliseconds), but because H\textsubscript{2} is small enough that we can see every
step, check every number, and build intuition for what happens at larger
scale. Later, when we work with H\textsubscript{2}O, the same pipeline will produce
...
For H\textsubscript{2}, this means: - \textbf{Two protons} (charge \(+e\) each),
...
We can't. Not for H\textsubscript{2}, not for anything with more than one electron. The
...
For H\textsubscript{2} at the equilibrium bond length \(R = 0.7414\) Å (= 1.401 Bohr):
...
times, once per geometry. (This is exactly what our H\textsubscript{2}O bond-angle scan
...
\emph{within the chosen basis set}. STO-3G H\textsubscript{2} has only 4 spin-orbitals
...
H\textsubscript{2}}\label{molecular-orbitals-for-hux2082}
...
spin-up, \(\beta\) = spin-down), giving us 2 spatial orbitals $\times$ 2 spins
...
H\textsubscript{2}}\label{the-six-configurations-of-hux2082}
...
The \textbf{exact} ground state of H\textsubscript{2} is a \emph{superposition} of some
...
basis states \(\lvert q_0 q_1 q_2 q_3\rangle\). Four spin-orbitals $\rightarrow$
four qubits. Six configurations $\rightarrow$ a 16-dimensional Hilbert space (of
...
in the configuration basis and diagonalize it. For H\textsubscript{2}, that would work
fine. But it wouldn't scale --- for H\textsubscript{2}O with 14 spin-orbitals, the
...
\section{The Numbers: H\textsubscript{2} Integrals in
STO-3G}\label{the-numbers-hux2082-integrals-in-sto-3g}

For H\textsubscript{2} in STO-3G at the equilibrium bond length, the non-zero one-body
...
\item
  ...
  into a finite set of molecular orbitals. For H\textsubscript{2}, this gives 2 spatial
  orbitals $\rightarrow$ 4 spin-orbitals $\rightarrow$ 6 two-electron configurations.
\item
...
\item
  ...
  from those orbitals. More orbitals $\rightarrow$ exponentially more
  configurations.
\item
...
\item
  ...
  off by \textasciitilde0.7 Ha for H\textsubscript{2}.
\end{enumerate}
...
\item
  ...
  exist for H\textsubscript{2}O in a minimal (STO-3G) basis with 7 spatial orbitals (14
  spin-orbitals, 10 electrons)? How does this compare with H\textsubscript{2}?
\item
...
\item
  ...
  configurations would H\textsubscript{2} have?
\end{enumerate}
...
different integral (the exchange integral). For H\textsubscript{2} in STO-3G,
...
\section{A Complete Example: H\textsubscript{2} Two-Body
Integrals}\label{a-complete-example-hux2082-two-body-integrals}

Here are all the non-zero two-body integrals for H\textsubscript{2} in STO-3G, in
...
\tightlist
\item
  One-body key \texttt{"p,q"} $\rightarrow$ \(h_{pq}\)
\item
  Two-body key \texttt{"p,q,r,s"} $\rightarrow$ \(\langle pq \mid rs\rangle\)
\end{itemize}
...
\item
  \textbf{Error detection.} You compute a 4-qubit H\textsubscript{2} Hamiltonian and get
...
orbitals give four spin-orbitals. Seven spatial orbitals (H\textsubscript{2}O in STO-3G)
...
For the molecules in this book (H\textsubscript{2}, H\textsubscript{2}O), spin-orbit coupling is
...
For H\textsubscript{2}: 2 spatial orbitals $\rightarrow$ \textbf{4 spin-orbitals}. The one-body
...
\subsection{H\textsubscript{2} one-body integrals (spin-orbital
...
\(h^{\text{spatial}}_{00}\), same spin $\checkmark$ \\
1 (\(\sigma_g, \beta\)) & 1 (\(\sigma_g, \beta\)) & \(-1.2563\) &
\(h^{\text{spatial}}_{00}\), same spin $\checkmark$ \\
2 (\(\sigma_u, \alpha\)) & 2 (\(\sigma_u, \alpha\)) & \(-0.4719\) &
\(h^{\text{spatial}}_{11}\), same spin $\checkmark$ \\
...
\(\sigma_q = \sigma_s = \alpha\) & $\checkmark$ \\
\(\beta\beta - \beta\beta\) & \(\sigma_p = \sigma_r = \beta\) &
\(\sigma_q = \sigma_s = \beta\) & $\checkmark$ \\
...
\(\sigma_r = \alpha\) $\rightarrow$ same spin $\checkmark$ - Electron 2: \(\sigma_q = \beta\),
\(\sigma_s = \beta\) $\rightarrow$ same spin $\checkmark$
...
\NormalTok{    //}\CommentTok{ Same{-}spin alphaalpha{-}alphaalpha}
...
\NormalTok{    //}\CommentTok{ Same{-}spin betabeta{-}betabeta (identical values, odd indices)}
...
\NormalTok{    //}\CommentTok{ Cross{-}spin alphabeta{-}alphabeta (electron 1 = alpha, electron 2 = beta)}
...
\NormalTok{    //}\CommentTok{ Cross{-}spin betaalpha{-}betaalpha (electron 1 = beta, electron 2 = alpha)}
...
\NormalTok{//}\CommentTok{ Build the 4{-}qubit H2 Hamiltonian with Jordan{-}Wigner}
...
complex coefficients. It will have 15 terms (the famous H\textsubscript{2} Hamiltonian
...
\item
  ...
  two index transformations: spatial$\rightarrow$spin-orbital \emph{and}
  chemist$\rightarrow$physicist. It's easy to get one right and the other wrong. Use
...
\item
  ...
  integrals does H\textsubscript{2}/STO-3G have in total? How many of those are
  cross-spin? (Answer: 32 total, 16 cross-spin --- exactly half.)
\item
  \textbf{Missing block detection.} If you compute the H\textsubscript{2} Hamiltonian
...
\item
  ...
  using blocked indexing (\(0\alpha, 1\alpha, 0\beta, 1\beta\) $\rightarrow$ indices
  0, 1, 2, 3). Which entries of the matrix change? Do the eigenvalues
  change?
\item
  \textbf{H\textsubscript{2}O scale-up.} Water has 7 spatial orbitals in STO-3G (after
...
\item
  ...
  Hamiltonian). Basic familiarity with 2$\times$2 matrices is helpful but not
  essential.
\end{itemize}
...
Hilbert space of our H\textsubscript{2} problem. For 100 qubits:
...
Trapped ion & IonQ (Forte) & \textasciitilde600 $\mu$s &
\textasciitilde{}\(4 \times 10^{-3}\) &
\textasciitilde{}\(3 \times 10^{-4}\) \\
\end{longtable}

The pattern: CNOT is \textbf{10--100$\times$ slower} and \textbf{10--100$\times$
...
than enough for H\textsubscript{2} (4 qubits), H\textsubscript{2}O (12 qubits), and most textbook-scale
...
expensive. Circuit cost $\approx$ CNOT count. Everything else follows from this.
\end{quote}
...
4 & 6 & \(XXYY\) --- the H\textsubscript{2} exchange terms \\
12 & 22 & Typical JW term for H\textsubscript{2}O \\
...
\item
  ...
  10--100$\times$ more expensive than single-qubit gates. Circuit cost $\approx$ CNOT
  count.
\item
...
\item
  ...
  to qubit 0, then CNOT(0$\rightarrow$1). Write out the resulting state. Why is it
...
For H\textsubscript{2} with 4 spin-orbitals, this is a vector with 6 components (the 6
...
For H\textsubscript{2}O with 14 spin-orbitals and 10 electrons, the number of
...
vector \(\lvert 1100\rangle\) --- the Hartree--Fock ground state of H\textsubscript{2},
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-01.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-02.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-06.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-07.png}}
...
15-term H\textsubscript{2} Hamiltonian --- and see that they all agree.
...
\item
  ...
  strings, and how to read the resulting 15-term H\textsubscript{2} Hamiltonian as a map
...
A quantum state is fundamentally different. The ground state of H\textsubscript{2} is
...
\tightlist
\item
  \textbf{The chemistry} (Chapter 1): H\textsubscript{2} in STO-3G, 4 spin-orbitals, the
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-08.png}}
...
The non-zero one-body integrals for H\textsubscript{2} are all diagonal (Chapter 3):
...
\emph{between} the two Z\textsubscript{0} factors instead of outside them. Expanding
qubit 0:
...
\item
  ...
  off-diagonal terms $\rightarrow$ no correlation energy $\rightarrow$ no quantum advantage.
\item
  The H\textsubscript{2} Hamiltonian has 11 diagonal terms (Hartree--Fock) and 4
...
\item
  ...
  they all produce the same number of terms for H\textsubscript{2}?
\end{enumerate}
...
\emph{We built the H\textsubscript{2} Hamiltonian with Jordan--Wigner. But JW has a
...
linearly with system size: \(O(n)\). For H\textsubscript{2} with 4 qubits, the maximum
weight is 4 --- manageable. But for H\textsubscript{2}O with 12 active spin-orbitals
...
Let's find out empirically. Build the H\textsubscript{2} Hamiltonian with all six
...
For H\textsubscript{2} with 4 qubits, the Pauli strings are actually identical across
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-09.png}}
...
\item
  For H\textsubscript{2} (4 qubits), the encodings produce identical Pauli strings. The
...
\item
  \textbf{Encoding comparison for H\textsubscript{2}O.} Build the H\textsubscript{2}O/STO-3G Hamiltonian
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-10.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-11.png}}

\textbf{Balanced ternary tree} (\(n = 8\), midpoint-split, root = mode
4):

\pandocbounded{\includegraphics[keepaspectratio]{diagram-12.png}}
...
Build the H\textsubscript{2} Hamiltonian with the new encoding and compare eigenvalues
...
For H\textsubscript{2} in STO-3G, we have exact classical reference values. For larger
...
basis states, but H\textsubscript{2} has only 2 electrons --- so only 6 of those states
...
\section{The Eigenspectrum of H\textsubscript{2}}\label{the-eigenspectrum-of-hux2082}
...
This is the \textbf{exact} ground-state energy of H\textsubscript{2} in the STO-3G basis
...
intractable. For H\textsubscript{2} in STO-3G (6 configurations), any laptop can do Full
...
\KeywordTok{for} \OperatorTok{(}\NormalTok{name, encoder}\OperatorTok{)} \KeywordTok{in}\NormalTok{ encoders }\KeywordTok{do}
    \KeywordTok{let}\NormalTok{ ham = computeHamiltonianWith encoder h2Factory }\DecValTok{4}\NormalTok{u}
\NormalTok{    //}\CommentTok{ ... build 16x16 matrix, diagonalize ...}
\NormalTok{    printfn }\StringTok{"\%{-}25s  E0 = \%.10f Ha"}\NormalTok{ name groundStateEnergy}
\end{Highlighting}
\end{Shaded}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-13.png}}

The Hamiltonian is correct. It is the exact qubit representation of H\textsubscript{2}
...
\item
  The H\textsubscript{2}/STO-3G ground-state energy is \(E_0 = -1.1422\) Ha (Full CI,
  exact within basis).
\item
...
\item
  ...
  \(\lvert 0000\rangle\)?)
\item
  \textbf{Correlation energy.} Compute the Hartree--Fock energy of H\textsubscript{2} by
...
\item
  ...
  gives the Full CI eigenvalues for H\textsubscript{2}/STO-3G.
\item
  Helgaker, T., Jørgensen, P., and Olsen, J. \emph{Molecular
...
\tightlist
\item
  \textbf{What you'll learn:} How fermion-to-qubit encoding creates Z\textsubscript{2}
...
Look again at the 15-term H\textsubscript{2} Hamiltonian from Chapter 6. Focus on the
...
diagonal (I/Z only) across all 15 terms. So H\textsubscript{2} under JW has \textbf{no
diagonal Z\textsubscript{2} symmetries} at first glance.

But this is specific to H\textsubscript{2}'s structure and the JW encoding. Many
...
diagonal, there may be \textbf{multi-qubit} Z\textsubscript{2} symmetries (like
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-14.png}}

\begin{longtable}[]{@{}lcl@{}}
\toprule\noalign{}
What shrinks & Factor & Example (12 $\rightarrow$ 9 qubits) \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Qubit count & \(-k\) & 3 fewer physical qubits needed \\
Hilbert space & \(2^{-k}\) & \(4096 \to 512\) (8$\times$ smaller) \\
...
\subsection{Level 1: Diagonal Z\textsubscript{2}}\label{level-1-diagonal-zux2082}
...
\tightlist
\item
  Encoded Hamiltonians often have Z\textsubscript{2} symmetries --- qubits whose value
...
\item
  Diagonal Z\textsubscript{2} symmetries (single-qubit Z generators) are the simplest
  case. General Z\textsubscript{2} symmetries require Clifford rotation.
\item
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Assuming H\textsubscript{2} has taperable qubits under JW.} It doesn't --- the
...
\item
  ...
  conservation laws might produce Z\textsubscript{2} symmetries? (Hint: total electron
...
\item
  ...
  H\textsubscript{2}O system, by what factor does the Hilbert space shrink? How many
...
\chapter{Chapter 11: Diagonal Z\textsubscript{2}
...
\tightlist
\item
  \textbf{What you'll learn:} The algorithm for detecting diagonal Z\textsubscript{2}
...
\item
  ...
  valuable and what Z\textsubscript{2} symmetries are).
\end{itemize}
...
say qubit \(j\) is \textbf{diagonal Z\textsubscript{2} symmetric} if:

\[\forall \alpha:\; \sigma_\alpha[j] \in \{I, Z\}\]

The algorithm is a single scan:

\pandocbounded{\includegraphics[keepaspectratio]{diagram-15.png}}

\textbf{Complexity:} \(O(n \times m)\) where \(n\) is the qubit count
and \(m\) is the number of terms. For H\textsubscript{2}O with 12 active spin-orbitals
...
For each diagonal Z\textsubscript{2} qubit, we \textbf{choose} whether to fix its \(Z\)
...
number (even vs odd), then: - Sector \(+1\) $\rightarrow$ even number of electrons -
Sector \(-1\) $\rightarrow$ odd number of electrons
...
Four qubits $\rightarrow$ two qubits. The eigenvalues of \(\hat{H}'\) are exactly
...
\item
  ...
  is the ground-state energy. For H\textsubscript{2} with 2 diagonal qubits, this means
...
\NormalTok{        printfn }\StringTok{"Sector (\%+d, \%+d): E0 = \%.6f Ha"}\NormalTok{ s0 s1 e0}
\end{Highlighting}
\end{Shaded}
...
\emph{When no single qubit is diagonal, multi-qubit Z\textsubscript{2} symmetries may
...
\item
  ...
  strings, how to find all Z\textsubscript{2} symmetry generators via GF(2) linear
  ...
  everything.
\item
  \textbf{Why this matters:} Many molecular Hamiltonians have Z\textsubscript{2}
...
is a valid Z\textsubscript{2} symmetry generator --- it just involves \emph{two} qubits
instead of one.
...
To find all Z\textsubscript{2} generators systematically, we need a way to represent
...
A Pauli string \(g\) is a Z\textsubscript{2} symmetry of \(\hat{H}\) if it commutes with
...
\textbf{null space} of the commutation check matrix --- gives all Z\textsubscript{2}
generators.
...
is the number of independent Z\textsubscript{2} symmetries.
...
\item
  ...
  per term).
\item
  \textbf{Compute} its null space via binary Gaussian elimination $\rightarrow$
...
\bottomrule\noalign{}
\endlastfoot
Hadamard & \(H_j\) & Swaps X$\leftrightarrow$Z on qubit \(j\) \\
Phase gate & \(S_j\) & Maps X$\rightarrow$Y on qubit \(j\) (Z unchanged) \\
...
\NormalTok{result.Generators          //}\CommentTok{ the Z2 generators found}
...
\textbf{Step 1: Check for diagonal Z\textsubscript{2} qubits.}
...
\textbf{Step 2: Find general Z\textsubscript{2} generators.}
...
\item
  ...
  even $\rightarrow$ \textbf{commute} $\checkmark$
\item
  Does \(Z_0 Z_1\) commute with \(Y_0 Y_1\)? X-bits of \(YY\) are
  \((1,1)\), Z-bits are \((1,1)\). Crosswise sum:
  \((0 \cdot 1 + 1 \cdot 1) + (0 \cdot 1 + 1 \cdot 1) = 2\) $\rightarrow$
  \textbf{commute} $\checkmark$
\item
  Does \(Z_0 Z_1\) commute with itself? Always yes $\checkmark$
\end{itemize}

\(Z_0 Z_1\) is a Z\textsubscript{2} generator. One independent generator $\rightarrow$ one qubit can
be tapered.
...
The key gate: \textbf{CNOT(1 $\rightarrow$ 0)} (qubit 1 is control, qubit 0 is
...
removes it). We need the reverse: \textbf{CNOT(0 $\rightarrow$ 1)} (qubit 0 is
...
\bottomrule\noalign{}
\endlastfoot
\(X_0 X_1\) & \(X_0 X_1\) $\rightarrow$ actually transforms to
...
A 2-qubit problem reduced to 1 qubit --- a 2$\times$ Hilbert space reduction
...
\item
  ...
  binary Gaussian elimination) gives all Z\textsubscript{2} generators.
\item
...
\item
  ...
  Hamiltonians.'' arXiv:1701.08213 (2017). The general Z\textsubscript{2} tapering
  framework.
\item
...
Hilbert space shrank by 4$\times$, which is the primary gain.

\subsection{Heisenberg model (Clifford
needed)}\label{heisenberg-model-clifford-needed}

\(\hat{H} = XX + YY + ZZ\) on 2 qubits. No diagonal Z\textsubscript{2} symmetries, but
\(Z_0Z_1\) is a general Z\textsubscript{2} generator.
...
\bottomrule\noalign{}
\endlastfoot
Symmetries found & 0 & $\geq$1 \\
Qubits removed & 0 & $\geq$1 \\
\end{longtable}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-16.png}}

For a 14-qubit H\textsubscript{2}O system (STO-3G, frozen core):
...
The combination of tapering (14$\rightarrow$11 qubits) and ternary tree encoding
(weight 14$\rightarrow$4) gives roughly a 5$\times$ total reduction in CNOT cost.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{Tapering Stage Complete}\label{tapering-stage-complete}

\pandocbounded{\includegraphics[keepaspectratio]{diagram-17.png}}
...
you exhaust all Z\textsubscript{2} symmetries (operators that square to the identity and
commute with every Hamiltonian term).

Could you do more? In principle, yes. The Hamiltonian has other
symmetries beyond Z\textsubscript{2} --- particle-number conservation (U(1)), spin
...
In practice, Z\textsubscript{2} tapering removes 2 qubits from H\textsubscript{2} (4 $\rightarrow$ 2), 3 from H\textsubscript{2}O
(14 $\rightarrow$ 11), and typically 3--5 from larger molecules. The absolute
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-18.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-19.png}}

\textbf{Trade-off:} More Trotter steps (\(N\)) $\rightarrow$ better approximation
but deeper circuit. Fewer steps $\rightarrow$ shallower circuit but larger error.
...
For the molecules in this book (H\textsubscript{2}, H\textsubscript{2}O), Trotterization is more than
...
\item
  ...
  Trotter decomposition to the H\textsubscript{2} Hamiltonian, what the rotation list
...
H\textsubscript{2}}\label{what-happens-when-theory-meets-hux2082}
...
\section{The H\textsubscript{2} Hamiltonian, One Last
...
\NormalTok{//}\CommentTok{ -> 28 rotations (2 x 14)}
\end{Highlighting}
\end{Shaded}
...
\bottomrule\noalign{}
\endlastfoot
Rotations per step & \(L\) (14 for H\textsubscript{2}) & \(2L\) (28 for H\textsubscript{2}) \\
...
$\rightarrow$ bad approximation. Too small $\rightarrow$ too many steps $\rightarrow$ too deep a circuit.

A practical rule of thumb: \(\Delta t \leq 1 / \lVert\hat{H}\rVert_1\),
where the 1-norm is \(\lVert\hat{H}\rVert_1 = \sum_k |c_k|\).

For H\textsubscript{2}: \(\lVert\hat{H}\rVert_1 \approx 3.7\) Ha, giving
\(\Delta t \lesssim 0.27\). Our choice of \(\Delta t = 0.1\) is
comfortably within this bound.

For H\textsubscript{2}O (14 spin-orbitals, or about 11 qubits after tapering):
...
For H\textsubscript{2} (14 non-identity terms):
...
H\textsubscript{2}. For H\textsubscript{2}O with \textasciitilde600 terms and higher weights, the
...
\subsection{Worked example: H\textsubscript{2} at chemical
accuracy}\label{worked-example-hux2082-at-chemical-accuracy}

For H\textsubscript{2}: \(\lVert H \rVert_1 \approx 3.7\) Ha. Target precision:
...
because many H\textsubscript{2} terms commute.
...
For H\textsubscript{2}, even the conservative estimate says \textasciitilde52
second-order steps suffice for chemical accuracy --- about 3,700 CNOTs.
For H\textsubscript{2}O (\(\lVert H \rVert_1 \approx 30\) Ha), the step count grows and
...
improve on the 1-norm estimate by a factor of 5--50$\times$, because real
...
\item
  ...
  \(\sum_k 2(w_k - 1)\).
\item
  For H\textsubscript{2}: 36 CNOTs per first-order step. Feasible. For H\textsubscript{2}O: thousands.
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Rotation count.} For H\textsubscript{2}O with 600 non-identity terms, how many
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-20.png}}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-21.png}}
...
\textbf{CNOT staircase:} CNOT(0$\rightarrow$1), CNOT(1$\rightarrow$2), CNOT(2$\rightarrow$3)

\textbf{Rz:} \(R_z(2\theta)\) on qubit 3

\textbf{Reverse CNOT staircase:} CNOT(2$\rightarrow$3), CNOT(1$\rightarrow$2), CNOT(0$\rightarrow$1)
...
The 6$\times$ ratio between JW and TT at \(n=32\) is entirely due to the CNOT
...
\tightlist
\item
  Each Pauli rotation decomposes into basis change $\rightarrow$ CNOT staircase $\rightarrow$
  \(R_z\) $\rightarrow$ reverse.
\item
...
\tightlist
\item
  \textbf{What you'll learn:} The complete CNOT cost for H\textsubscript{2} and H\textsubscript{2}O
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-22.png}}
...
\section{H\textsubscript{2} (4 qubits, 15 terms)}\label{hux2082-4-qubits-15-terms}
...
\section{H\textsubscript{2}O (12 qubits, frozen core,
STO-3G)}\label{hux2082o-12-qubits-frozen-core-sto-3g}

This is where the differences become real. H\textsubscript{2}O has 7 spatial orbitals
...
\textasciitilde3.6$\times$ fewer CNOTs than JW without tapering. For a 100-step
...
\bottomrule\noalign{}
\endlastfoot
H\textsubscript{2} & 4 & 36 & 40 & 0.9$\times$ \\
LiH & 12 & \textasciitilde1200 & \textasciitilde500 & 2.4$\times$ \\
H\textsubscript{2}O & 14 & \textasciitilde1800 & \textasciitilde600 & 3$\times$ \\
N\textsubscript{2} & 20 & \textasciitilde5000 & \textasciitilde1200 & 4.2$\times$ \\
FeMo-co & \textasciitilde100 & \textasciitilde500,000 &
\textasciitilde20,000 & \textasciitilde25$\times$ \\
\end{longtable}
...
(\textasciitilde100 orbitals), the 25$\times$ CNOT reduction from encoding
...
\item
  ...
  3--5$\times$ CNOTs over JW.
\item
  For large molecules (\(n \sim 100\)), the savings are
  \textasciitilde25$\times$ --- potentially enabling experiments that would
  otherwise be infeasible.
\item
  The complete optimization stack is: \textbf{taper $\rightarrow$ encode $\rightarrow$
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{H\textsubscript{2} by hand.} Verify the 36-CNOT figure for H\textsubscript{2} first-order
...
transformation, implemented it, and tested it on H\textsubscript{2}.

Now we put the whole chain together. The script below takes the H\textsubscript{2}
...
the energy-evaluation backend. For H\textsubscript{2} (4 qubits,
\(2^4 = 16\)-dimensional matrix) and H\textsubscript{2}O in minimal basis
...
\NormalTok{    //}\CommentTok{ Step 3: Trotterize --- one first{-}order step at Deltat = 0.1}
...
\textbf{\texttt{taper\ defaultTaperingOptions\ ham}} --- Finds Z\textsubscript{2}
...
the positive sector and Clifford-based tapering. For JW on H\textsubscript{2}, this
...
The Pauli strings are different, but the weights are the same. H\textsubscript{2} was
always our \emph{teacher}, not our \emph{benchmark}. The differences
appear at scale --- we'll see them with H\textsubscript{2}O in the next chapter.
...
\section{Extra Credit: The H\textsubscript{2} Dissociation
...
potential energy surface of H\textsubscript{2} as the two atoms are pulled apart.
...
precomputed Pauli strings by numbers. For a 20-point scan, this is 20$\times$
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{h2_dissociation.png}}
\caption{H\textsubscript{2} dissociation curve (STO-3G): Hartree--Fock vs Full CI. The
FCI curve correctly dissociates while HF fails at large bond lengths.}
\end{figure}

This is the same kind of scan we'll do for H\textsubscript{2}O's bond angle in Chapter
...
energy \(D_e\) = 1.137 - 0.933 = 0.204 Ha $\approx$ 5.6 eV, compared to the
...
equilibrium geometry because H\textsubscript{2} has no lone pairs or angular structure
to distort.
...
\item
  The H\textsubscript{2} dissociation curve --- equilibrium bond length, dissociation
...
\item
  \textbf{Prerequisites:} Chapter 18 (the complete H\textsubscript{2} pipeline).
\end{itemize}
...
angle of 104.52\textdegree{}. The oxygen atom sits at the vertex, the two hydrogen
...
``less than 109.5\textdegree{}'' and leaves it at that.
...
\tightlist
\item
  \textbf{Pick a range of angles.} We'll scan from 60\textdegree{} to 180\textdegree{} in 5\textdegree{}
  steps (25 geometries).
\item
...
value. For a 25-point scan, this is 25$\times$ faster than calling
...
For H\textsubscript{2} we had 4 spin-orbitals and 16 integrals. For H\textsubscript{2}O in a minimal
...
\begin{quote}
\textbf{Active space and frozen core:} H\textsubscript{2}O has 10 electrons, but the
oxygen 1s core electrons are tightly bound and contribute negligibly to
chemical bonding. In a minimal basis (STO-3G, 7 spatial orbitals $\rightarrow$ 14
...
\KeywordTok{def}\NormalTok{ h2o\_integrals(angle\_degrees, bond\_length}\OperatorTok{=}\FloatTok{0.9584}\NormalTok{):}
    \CommentTok{"""Generate H2O integrals at a given H{-}O{-}H bond angle."""}
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-23.png}}
...
\NormalTok{    //}\CommentTok{ exactGroundStateEnergy builds the 2\^{}n x 2\^{}n matrix and returns min(eigenvalues).}
    \KeywordTok{let}\NormalTok{ energy = exactGroundStateEnergy tapered.Hamiltonian}

\NormalTok{    printfn }\StringTok{"\%.0fdeg  \%.6f Ha"}\NormalTok{ angle energy}
\end{Highlighting}
\end{Shaded}
...
\begin{longtable}[]{@{}cccc@{}}
\toprule\noalign{}
Angle (\textdegree{}) & \(E\) (Ha) & Angle (\textdegree{}) & \(E\) (Ha) \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
...
The minimum is near \textbf{100\textdegree{}}, with energy dropping steeply on both
sides. The coarse scan tells us \emph{where to look} --- but the minimum
could be anywhere between 95\textdegree{} and 105\textdegree{}. This is exactly how a
...
We re-run the scan from 95\textdegree{} to 115\textdegree{} in 1\textdegree{} steps. The skeleton is already
...
The minimum is at \textbf{99\textdegree{}} with energy \textbf{-75.0141 Ha}. The
curve is clearly parabolic --- the energy changes by only 0.00005 Ha
between 98\textdegree{} and 100\textdegree{}, then drops away more steeply on either side.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio]{h2o_bond_angle.png}}
\caption{H\textsubscript{2}O bond angle scan (STO-3G, FCI): coarse scan (left)
identifies the minimum near 100\textdegree{}; fine scan (right) pinpoints it at
99\textdegree{}.}
\end{figure}
...
The minimum at \textbf{99\textdegree{}} in STO-3G is \textasciitilde5\textdegree{} below the
experimental value of 104.52\textdegree{}. The discrepancy comes from the minimal
...
cc-pVTZ level, the computed angle is within 0.5\textdegree{} of experiment.
...
At 180\textdegree{} (linear geometry), the molecular orbitals have a symmetry that
...
With 14 spin-orbitals, H\textsubscript{2}O is the first molecule in this book where
...
gives a more accurate simulation for the same hardware budget. At H\textsubscript{2}O
scale, that's roughly a 3$\times$ reduction in two-qubit gates.
...
A linear H\textsubscript{2}O molecule would have no permanent dipole and a very
...
\item
  The energy minimum of the H\textsubscript{2}O bond angle scan occurs at \textbf{99\textdegree{}}
  in STO-3G --- close to the experimental 104.52\textdegree{}, with the discrepancy
...
smallest eigenvalue classically. For H\textsubscript{2} (\(2^4 = 16\)-dimensional
matrix) and H\textsubscript{2}O in minimal basis (\(2^{11} = 2{,}048\)-dimensional after
tapering), exact diagonalisation is trivially feasible on a laptop. But
for larger systems --- N\textsubscript{2} (20 qubits $\rightarrow$ \(2^{20} \approx 10^6\)) or
FeMo-co (108 qubits $\rightarrow$ \(2^{108} \approx 10^{32}\)) --- the matrix does
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-24.png}}
...
For the 15-term H\textsubscript{2} Hamiltonian, this typically produces 5 measurement
groups --- a 3$\times$ reduction in the number of distinct circuits needed.
...
For H\textsubscript{2}: \(\sum|c_k| \approx 3.7\) Ha, giving \textasciitilde5 million
shots at chemical accuracy. For H\textsubscript{2}O: the 1-norm is larger, and the shot
...
For H\textsubscript{2} at chemical accuracy (\textasciitilde10 bits of precision): -
System: 2--4 qubits (depending on tapering) - Ancilla: 10 qubits -
Total: \textasciitilde14 qubits, \textasciitilde10,000 CNOTs

For H\textsubscript{2}O at chemical accuracy: - System: \textasciitilde11 qubits (after
tapering) - Ancilla: 10 qubits - Total: \textasciitilde21 qubits,
\textasciitilde500,000 CNOTs

The 50$\times$ jump from H\textsubscript{2} to H\textsubscript{2}O is driven by two factors: more qubits (more
...
Let's look at the measurement infrastructure for H\textsubscript{2} in detail. The
...
\texttt{Gate.Rz\ (i,\ $\theta$)} & \texttt{rz($\theta$)\ q{[}i{]};} \\
\end{longtable}
...
Here is the full pipeline for H\textsubscript{2}, exporting to all three formats:
...
\item
  ...
  increase CNOT count by 2--5$\times$.
\item
...
For H\textsubscript{2}, the exact FCI energy is \(-1.1373\) Ha (electronic only) or
...
\chapter{Chapter 22: Scaling --- From H\textsubscript{2} to
FeMo-co}\label{chapter-22-scaling-from-hux2082-to-femo-co}

\emph{H\textsubscript{2} was our teacher. H\textsubscript{2}O was our first real test. Now we look at
...
Growth
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
H\textsubscript{2} (\(n{=}4\))
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
H\textsubscript{2}O (\(n{=}14\))
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
N\textsubscript{2} (\(n{=}20\))
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
...
\bottomrule\noalign{}
\endlastfoot
H\textsubscript{2} & 4 & 12 & 12 & 1.0$\times$ \\
LiH & 12 & \textasciitilde400 & \textasciitilde200 & 2.0$\times$ \\
H\textsubscript{2}O & 14 & \textasciitilde1,800 & \textasciitilde600 & 3.0$\times$ \\
N\textsubscript{2} & 20 & \textasciitilde8,000 & \textasciitilde2,000 & 4.0$\times$ \\
FeMo-co & \textasciitilde108 & \textasciitilde{}\(10^7\) &
\textasciitilde{}\(10^5\) & \textasciitilde100$\times$ \\
\end{longtable}
...
matching. The estimates for N\textsubscript{2} and FeMo-co are order-of-magnitude
...
At \(\varepsilon = 0.5\%\) and 1,800 CNOTs (JW for H\textsubscript{2}O),
\(P_\text{success} \approx 0.01\%\). At 600 CNOTs (TT for H\textsubscript{2}O),
\(P_\text{success} \approx 5\%\). Under these typical assumptions,
that's roughly a 500$\times$ improvement in success probability from encoding
...
\bottomrule\noalign{}
\endlastfoot
H\textsubscript{2} & 4 & 2 & 15 & 5 \\
H\textsubscript{2}O & 14 & \textasciitilde11 & \textasciitilde600 &
\textasciitilde300 \\
N\textsubscript{2} & 20 & \textasciitilde16 & \textasciitilde2,000 &
\textasciitilde1,200 \\
\end{longtable}
...
\section{Application: Encoding Choice at H\textsubscript{2}O
Scale}\label{application-encoding-choice-at-hux2082o-scale}

With 14 spin-orbitals, H\textsubscript{2}O is the smallest molecule where encoding
...
The 3$\times$ reduction from JW to TT is the difference between a circuit that
...
roughly a 10$\times$ higher success probability per shot. Over millions of VQE
shots, that translates directly to better energy estimates.

This is the practical answer to ``which encoding should I use?'' --- at
H\textsubscript{2}O scale and beyond, ternary tree with tapering is the best option in
the FockMap toolkit.
...
atmospheric N\textsubscript{2} to ammonia --- and understanding its mechanism could
...
\textasciitilde108 system + \textasciitilde10 ancilla $\approx$ 120 logical
...
developed for H\textsubscript{2} (4 qubits, 15 terms, 12 CNOTs) is the \emph{same code}
...
\item
  ...
  doesn't yet exist.
\item
  The same pipeline code works at every scale --- from H\textsubscript{2} to FeMo-co.
...
\pandocbounded{\includegraphics[keepaspectratio]{diagram-25.png}}

Every arrow is a function call in FockMap. Every box has been tested on
H\textsubscript{2} and applied to H\textsubscript{2}O. The pipeline is real, open-source, and published
on NuGet.
...
FockMap's contribution is the symmetry information: the Z\textsubscript{2} generators,
...
ladder operators $\rightarrow$ Pauli strings $\rightarrow$ Hamiltonian assembly. The main
...
\item
  ...
  stabilisers, exactly as our tapering step measures the Z\textsubscript{2} generators.
\item
...
\item
  \textbf{Tapering beyond Z\textsubscript{2}}: our tapering removes symmetries that
  square to the identity (Z\textsubscript{2} symmetries). What about higher-order
...
The pipeline runs on a laptop for small molecules (H\textsubscript{2}, LiH). It produces
circuits that near-term hardware can attempt for medium molecules (H\textsubscript{2}O,
N\textsubscript{2}). And it generates the circuits that fault-tolerant hardware will
need for the molecules that actually matter (FeMo-co, transition-metal
catalysts, photochemical systems).

The H\textsubscript{2} dissociation curve in Chapter 18 showed the pipeline's
...
\chapter{Appendix: Library Cookbook
Reference}\label{appendix-library-cookbook-reference}
...
& 1 \\
\texttt{C\textless{}\textquotesingle{}T\textgreater{}} & Coefficient $\times$
single operator & 2 \\
...
& 1, 6 \\
\texttt{EncodingScheme} & Three index-set functions $\rightarrow$ custom encoding &
8 \\
...
\texttt{LadderOperatorUnit\ $\rightarrow$\ uint32\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& JW encoding \\
\texttt{bravyiKitaevTerms} &
\texttt{LadderOperatorUnit\ $\rightarrow$\ uint32\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& BK encoding \\
\texttt{parityTerms} &
\texttt{LadderOperatorUnit\ $\rightarrow$\ uint32\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& Parity encoding \\
\texttt{balancedBinaryTreeTerms} &
\texttt{LadderOperatorUnit\ $\rightarrow$\ uint32\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& Binary tree encoding \\
\texttt{ternaryTreeTerms} &
\texttt{LadderOperatorUnit\ $\rightarrow$\ uint32\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& Ternary tree encoding \\
\texttt{vlasovTreeTerms} &
\texttt{LadderOperatorUnit\ $\rightarrow$\ uint32\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& Vlasov tree encoding \\
\texttt{encodeOperator} & \texttt{EncodingScheme\ $\rightarrow$\ ...} & Custom
encoding \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{computeHamiltonianWith} &
\texttt{EncoderFn\ $\rightarrow$\ CoefficientFactory\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& Build Hamiltonian with any encoding \\
\texttt{computeHamiltonianWithParallel} &
\texttt{EncoderFn\ $\rightarrow$\ CoefficientFactory\ $\rightarrow$\ uint32\ $\rightarrow$\ PauliRegisterSequence}
& Parallel version \\
\texttt{computeHamiltonianSkeleton} &
\texttt{EncoderFn\ $\rightarrow$\ uint32\ $\rightarrow$\ HamiltonianSkeleton} & Pre-compute
Pauli structure \\
\texttt{applyCoefficients} &
\texttt{HamiltonianSkeleton\ $\rightarrow$\ CoefficientFactory\ $\rightarrow$\ PauliRegisterSequence}
& Dress skeleton with integrals \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{diagonalZ2SymmetryQubits} &
\texttt{PauliRegisterSequence\ $\rightarrow$\ int{[}{]}} & Find diagonal Z\textsubscript{2}
qubits \\
\texttt{taperDiagonalZ2} &
\texttt{(int\ $\times$\ int)\ list\ $\rightarrow$\ PauliRegisterSequence\ $\rightarrow$\ Z2TaperingResult}
& Apply diagonal tapering \\
\texttt{findCommutingGenerators} &
\texttt{PauliRegisterSequence\ $\rightarrow$\ SymplecticVector{[}{]}} & Find all Z\textsubscript{2}
symmetries \\
\texttt{taper} &
\texttt{TaperingOptions\ $\rightarrow$\ PauliRegisterSequence\ $\rightarrow$\ TaperingResult} &
Unified tapering pipeline \\
\end{longtable}
...
\bottomrule\noalign{}
\endlastfoot
\texttt{ConstructNormalOrdered} &
\texttt{S\textless{}IxOp\textless{}uint32,\ LadderOperatorUnit\textgreater{}\textgreater{}\ $\rightarrow$\ ...\ option}
& Fermionic (CAR) normal ordering \\
\texttt{constructBosonicNormalOrdered} &
\texttt{S\textless{}IxOp\textless{}uint32,\ LadderOperatorUnit\textgreater{}\textgreater{}\ $\rightarrow$\ ...\ option}
& Bosonic (CCR) normal ordering \\
\texttt{constructMixedNormalOrdered} &
\texttt{S\textless{}IxOp\textless{}uint32,\ SectorLadderOperatorUnit\textgreater{}\textgreater{}\ $\rightarrow$\ ...\ option}
